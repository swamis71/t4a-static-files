...
wp.customize.controlConstructor['typography'] = wp.customize.Control.extend( {
    ready: function() {
		...
		renderSubControl = function( fontFamily, sub, startValue ) {
			...
			_.each( travel_booking_pro_all_fonts, function( font, i ) {
			...
			if ( false === isStandard || 'subsets' !== sub ) {
				...
				}).data( 'travel-booking-pro-selectize' );
			...
			if ( true === isStandard ) {
				...
				if ( 2 > subList.length ) {
					...
					control.container.find( '.travel-variant-wrapper' ).css( 'display', 'none' );
				} else {
					control.container.find( '.travel-variant-wrapper' ).css( 'display', 'block' );
				}
		...
		jQuery( fontFamilySelector ).selectize({
			options:     travel_booking_pro_all_fonts,
