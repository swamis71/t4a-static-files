	 ...
	 */
	 function get_api_response( $api_params ) {
		...
		if ( is_wp_error( $response ) ) {
			wp_die( $response->get_error_message(), __( 'Error', 'travel-booking-pro' ) . $response->get_error_code() );
		}
	...
	function activate_license() {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				$message = __( 'An error occurred, please try again.', 'travel-booking-pro' );
			}
			...
			if ( false === $license_data->success ) {
				...
				switch( $license_data->error ) {
					...
					case 'expired' :
						...
						$message = sprintf(
							__( 'Your license key expired on %s.', 'travel-booking-pro' ),
					...
					case 'revoked' :
						...
						$message = __( 'Your license key has been disabled.', 'travel-booking-pro' );
						break;

					case 'missing' :

						$message = __( 'Invalid license.', 'travel-booking-pro' );
						break;

					case 'invalid' :
					case 'site_inactive' :

						$message = __( 'Your license is not active for this URL.', 'travel-booking-pro' );
						break;

					case 'item_name_mismatch' :

						$message = sprintf( __( 'This appears to be an invalid license key for %s.', 'travel-booking-pro' ), $args['name'] );
						break;

					case 'no_activations_left':

						$message = __( 'Your license key has reached its activation limit.', 'travel-booking-pro' );
						break;
